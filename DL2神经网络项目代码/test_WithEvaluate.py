...
def get_feature_importance(feature_data, label_data, k =4,column = None):#对数据层面的改造
    """
    ...
    print('x_new', X_new)#计算相关系数
    scores = model.scores_ # scores即每一列与结果的相关性
    # 按重要性排序，选出最重要的 k 个
    indices = np.argsort(scores)[::-1]        #[::-1]表示反转逆置一个列表或者矩阵。
...
class CovidDatatest(Dataset):  # 用于加载数据集
    def __init__(self, file_path, mode="train", all_feature=True, feature_dim=6):  # 初始化,设置为训练模式  用mode来区分训练集还是测试集
        with open(file_path, "r") as f:  # 打开记录数据的文件
            ori_data = list(csv.reader(f))  # 将文件中的数据读取出来
            column = ori_data[0]
            csv_data = np.array(ori_data[1:])[:, 1:].astype(
                float)  # 不要第一列,去掉第一行第一列  将数据转化为numpy数组  astype是将数据转化为float类型
        feature = np.array(ori_data[1:])[:, 1:-1]#特征
        lable_data = np.array(ori_data[1:])[:, -1]#标签数据
        if all_feature:#判断是不是取所有列
        ...
        if mode == "train":  # 训练集
            ...
            indices = [i for i in range(len(csv_data))]  # 测试集需要取最后一列，不需要-1操作
...
def evaluate(save_path, test_loader, device, rel_path):
    ...
    with open(rel_path, "w", newline='') as f:  # 打开结构文件
        csvWriter = csv.writerow(["id", "tested_positive"])#将预测结果保存到指定csv文件中，每行包含一个索引和对应的预测值

        for i, value in enumerate(rel):#取数据的下标的值
            csvWriter.writerow([str(i), str(value)])#参数为下标和结果
    print("文件已保存到{}".format((rel_path)))

# 超参
def mseLoss_with_reg(pred, target, model):#loss = loss+w*w
...
train_dataset = CovidDatatest(train_file, "train",all_feature=all_feature, feature_dim=feature_dim)  # 将部分数据集数据转为一个包含训练数据的训练集对象
val_dataset = CovidDatatest(train_file, "train",all_feature=all_feature, feature_dim=feature_dim)   # 将部分训练集数据转为一个包含验证数据的验证集对象
test_dataset = CovidDatatest(test_file, "train",all_feature=all_feature, feature_dim=feature_dim)   # 将测试集数据转为一个包含测试数据的测试集对象
...
#对模型进行更改（all_feature, feature_dim
model = MyModel(inDim=feature_dim).to(device)  # 定义模型，输入参数为维度，将模型移动到设备（cpu/gpu）上
loss = mseLoss_with_reg  # 采用正则项，进一步优化 （计算均方误差，用于回归任务）
